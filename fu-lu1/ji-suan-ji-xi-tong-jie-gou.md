...
总线协议（本节将直接连接到 CPU 的通道称为总线，而总线在本质上也是一种通道）。现代 x86 计算机通常仅存在三种总线：**USB**、**SATA**、**PCIe**。
...
- `microSDXC`：表示该卡容量位于 64 GB–2 TB 范围内；4 GB–32 GB 的规格称为 microSDHC。其余规格目前已基本见不到——容量不超过 2 GB（microSD）或容量大于 2 TB（microSDUC）。该参数在实际选购中意义不大，因为存储卡本身已明确标注了容量大小。SD 协会（SD Association）定义：SDHC 标准为超过 2 GB 至 32 GB，SDXC 标准为超过 32 GB 至 2 TB。
