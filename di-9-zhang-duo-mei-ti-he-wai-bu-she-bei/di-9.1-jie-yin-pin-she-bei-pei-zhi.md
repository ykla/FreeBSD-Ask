...
上文中的 `pcm6: <Realtek ALC892 (Rear Digital)> (play)` 为数字输出接口。一般集成声卡的模拟接口采样率最高为 48 kHz，数字接口则绕过了板载 DAC 的采样率限制，允许使用支持更高采样率的外部 DAC，最高采样率可达到 192 kHz。若存在数字输出但机箱面板无对应接口，而主板提供 S/PDIF（Sony/Philips Digital Interface Format，索尼/飞利浦数字接口格式）插针，可安装 S/PDIF 挡板（通常包含光纤口和同轴口）。连接后即可使用。
