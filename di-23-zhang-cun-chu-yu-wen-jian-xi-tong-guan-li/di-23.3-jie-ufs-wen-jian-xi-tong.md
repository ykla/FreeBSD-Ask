...
需要明确区分的是，本节所述的 UFS 文件系统与手机等设备中使用的 UFS 存储属于完全不同的技术范畴。后者全称为 Universal Flash Storage（通用闪存存储），是一种硬件存储标准，目前已发展至 4.0 版本（FreeBSD 在 10.4 版本中支持 eMMC；而 UFS 存储硬件在 FreeBSD 15.0 的开发计划中，尚未得到支持）。
