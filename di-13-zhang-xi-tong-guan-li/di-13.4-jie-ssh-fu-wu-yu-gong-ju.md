...
OpenSSH 7.0 及以上版本默认禁用了 ssh-dss（DSA）公钥算法。FreeBSD 13.0 及后续版本内置的 OpenSSH 版本均不低于 8.4，可直接使用默认值生成密钥。
...
> **注意**
>
> SSH 客户端不支持 `用户名@IP:端口` 这种冒号语法指定端口（该写法仅适用于 scp 等命令）。如需在连接字符串中嵌入端口号，可使用 URI 格式 `ssh://用户名@IP:端口`。
